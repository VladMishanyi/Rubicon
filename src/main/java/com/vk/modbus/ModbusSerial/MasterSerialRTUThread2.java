 ...
 */
public class MasterSerialRTUThread2 extends Thread{
    private long startTime = 0;
    private final ModbusMasterSerialModel modbusMasterSerialModel4 =
            new ModbusMasterSerialModel("COM4", 9600, 8, 1, 0, 300, 1);
    private ModbusMaster modbusMasterSerial4;
    ...
    private void checkInitMaster(){
        if ( (modbusMasterSerial4 == null) || !(modbusMasterSerial4.isInitialized()) ) {
            try {
                modbusMasterSerial4 = modbusMasterSerialModel4.getMaster();
                modbusMasterSerial4.init();
            }
            catch (Exception e){
                String message = e.getMessage();
                System.out.println("ModBus Init problem, COM №"+ modbusMasterSerialModel4.getPort()+ "--"+message);
            }
        }
    }
    ...
    public void run() {
        while (!Thread.currentThread().isInterrupted()){
            try {
                ...
                modbusFloat.readDataFromModBus(modbusMasterSerial4, 16 , batchRead, false, modbusLocator0);
